...
//__________________________________________________________________________________________
// Double_t getMedianOfNonZeros(Double_t* input, const Int_t dim) //Defined in checkShapeEta
// {
//   Double_t values[dim];
...
TH2* checkShapeEtaTree(TTree* tree = 0x0, Double_t resolutionFactorX = 1/*2 or 2.1*/, Double_t resolutionFactorY = 1/*2 or 2.1*/,
                       ...
                       TString path = ".", TString fileName = "", Bool_t useDoubleGaussFit = kFALSE,
                       TString pathNameSplinesFile = "TPCPIDResponse.root", TString prSplinesName = "TSPLINE3_DATA_PROTON_LHC10H_PASS2_PBPB_MEAN",
                       Int_t returnMapType = kNoNormalisation, TString treeName = "fTree", TString etaTreeName = "bhess_PIDetaTree")
{

  const Int_t nOfMultChecks = 3;
  Int_t multCheckRanges[2*nOfMultChecks] = {0,200,400,600,800,1000};
  TH2F* hMultCheck[nOfMultChecks] = {0x0,0x0,0x0};

  for (Int_t i=0;i<nOfMultChecks;i++) {
    hMultCheck[i] = new TH2F(Form("hMultCheck_%i_%i",multCheckRanges[2*i],multCheckRanges[2*i+1]),Form("hMultCheck_%i_%i",multCheckRanges[2*i],multCheckRanges[2*i+1]), 18,-1,1,40,0.8,1.2);
    hMultCheck[i]->GetXaxis()->SetTitle("TanTheta");
    hMultCheck[i]->GetYaxis()->SetTitle("#Delta '");
    hMultCheck[i]->GetZaxis()->SetTitle("Entries");
  }

  if (!fileName.Contains(".root"))
    fileName = Form("%s%s.root", etaTreeName.Data(), fileName.Data());

  TString suffixForFileName = (TString)fileName(etaTreeName.Length(), fileName.Length() - etaTreeName.Length() - 5);
...
//    multiplicitySelectionString = "&& fMultiplicity > 3000 && fMultiplicity <= 3200";
  ...
  const Int_t clusterBinWidth = 20; // Other possibility: 20. Fits look better, but obviously the results for the PID-task are worse
  ...
  */
  ...
  Double_t fPtpcTest, fDeDxTest, fDeDxExpectedTest, fTanThetaTest, fTPCsignalNTest, fTPCsignalNsubthresholdTest, fNumTPCClustersInActiveVolumeTest, fPIDtypeTest = 0;
  Double_t fMultiplicityTest = 0;
    TTree *fTreeTest = new TTree("fTreeTest", "Tree for analysis of #eta dependence of TPC signal");
    fTreeTest->Branch("pTPCTest", &fPtpcTest);
    fTreeTest->Branch("dEdxTest", &fDeDxTest);
    fTreeTest->Branch("dEdxExpectedTest", &fDeDxExpectedTest);
    fTreeTest->Branch("tanThetaTest", &fTanThetaTest);
    fTreeTest->Branch("tpcSignalNTest", &fTPCsignalNTest);
    fTreeTest->Branch("pidTypeTest", &fPIDtypeTest);
    fTreeTest->Branch("fMultiplicityTest", &fMultiplicityTest);
    ...
    if (pidType != kMCid) {
      if ((pTPC < 0.6 && pidType != kTPCid)               // No V0s/TOF below 0.6 due to bad statistics
        //TODO NOW Maybe don't use this line since the tails get fewer V0's than the main peak, so the tails are overall reduced || (pTPC >= 0.6 && pTPC < pThresholdV0cut && pidType != kTPCandTOFid) || // Do not mix V0's and TOF in this range
          ...
          || (pTPC >= pThresholdV0plusTOFcut && pidType != kV0idPlusTOFaccepted)
) // Additionally require TOF
        continue;
    }
...
//    if (pidType==2) continue;
  ...
  fDeDxTest = dEdx;
  fDeDxExpectedTest = dEdxExpected;
  fTanThetaTest = tanTheta;
  fTPCsignalNTest = tpcSignalN;
  fPtpcTest = pTPC;
  fPIDtypeTest = pidType;
  fMultiplicityTest = fMultiplicity;
  fTreeTest->Fill();


  //  if(pTPC<pThresholdV0plusTOFcut && pTPC>pThresholdV0cut){
  //    cout << pidType <<endl;
  //  }
    ...
    for (Int_t i=0;i<nOfMultChecks;++i) {
      if (pTPC < 1.0 || pTPC > 1.2)
        continue;

      if (fMultiplicity > multCheckRanges[2*i] && fMultiplicity < multCheckRanges[2*i+1]) {
        hMultCheck[i]->Fill(tanTheta, dEdx / dEdxExpected);
      }
    }
  ...
  gROOT->LoadMacro("~/Documents/Splines/PIDcalib_software/checkShapeEta.C+");

  TObjArray* arr = new TObjArray();

  for (Int_t i=0;i<nOfMultChecks;++i) {
    FitSlicesY(hMultCheck[i],0.1,20,"QN",arr);
    arr->At(1)->Write();
  }

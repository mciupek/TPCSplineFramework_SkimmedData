...
void FitSlicesY(TH2 *hist, Double_t heightFractionForRange, Int_t cutThreshold, TString fitOption, TObjArray *arr)
{
  ...
  */
  ...
  hList[4] = (TH1D*)(hList[1]->Clone());
  hList[5] = (TH1D*)(hList[1]->Clone());
  ...
  for (Int_t ibin=axis->GetFirst(); ibin<=axis->GetLast(); ++ibin){
    ...
    const Double_t medianquantile = 0.5;
    Double_t median;

    h->GetQuantiles(1,&median,&medianquantile);

    hList[4]->SetBinContent(ibin,median);
    hList[5]->SetBinContent(ibin,h->GetMean());
                                    ...
                                    TString pathNameSplines = "splines_10h.pass2.root", TString splineName = "TSPLINE3_DATA_PROTON_LHC10H_PASS2_PBPB_MEAN")
{
  ...
  { // Fitting for all eta
    ...
    for (Int_t i = 0; i < numPbins; i++) {
      ...
      gSlParabelvsInvOffset->SetPoint(i, -10, 0);
      gSlParabelvsInvOffset->SetPointError(i, 0, 0);

      gCurvParabelvsInvOffset->SetPoint(i, -10, 0);
      gCurvParabelvsInvOffset->SetPointError(i, 0, 0);
      ...
      gSigmaSlopevsInvdEdxParabel->SetPoint(i, -10, 0);
      gSigmaSlopevsInvdEdxParabel->SetPointError(i, 0, 0);
      ...
      TH1D* hMedian = (TH1D*)arr.At(4);
      hMedian->SetNameTitle(Form("hMedian_allEta_pBin%d",i),Form("hMedian_allEta_pBin%d",i));
      hMedian->SetLineColor(kRed);

      TH1D* hMeanNotGaus = (TH1D*)arr.At(5);
      hMeanNotGaus->SetNameTitle(Form("hMeannotGaus_allEta_pBin%d",i),Form("hMeannotGaus_allEta_pBin%d",i));
      hMeanNotGaus->SetLineColor(kGreen);
      ...
      TFitResultPtr fitResTest = hMean->Fit("pol2","S N");
      TF1* stepfunc = new TF1("stepfunc","(x < (-[1]/(2*[2])) ? pol2(0) : [0] - [1]*[1]/(4*[2]))");
      stepfunc->SetParameter(0,fitResTest->GetParams()[0]);
      stepfunc->SetParameter(1,fitResTest->GetParams()[1]);
      stepfunc->SetParameter(2,fitResTest->GetParams()[2]);
      TFitResultPtr fitResstepFunc = hMean->Fit(stepfunc,"S","same",0,18000);

      hMean->DrawClone("same");
      hMedian->DrawClone("same");
      hMeanNotGaus->DrawClone("same");
        ...
        if (p0 > 0) {
          ...
          if (((Int_t)fitResSigma) != 0) {
            ...
            if (pSigma0 > 0) {
              ...
              // (positive slope makes physically no sense and is most likely due to uncertainties)
              if (pSigma1 < 0) {
      ...
      if (((Int_t)fitResstepFunc) != 0) {
        printf("Fit with step function failed!\n");
      }
      else {
        Double_t p0 = fitResstepFunc->GetParams()[0];
        Double_t p1 = fitResstepFunc->GetParams()[1];
        Double_t p2 = fitResstepFunc->GetParams()[2];
        Double_t err0 = fitResstepFunc->GetErrors()[0];
        Double_t err1 = fitResstepFunc->GetErrors()[1];
        Double_t err2 = fitResstepFunc->GetErrors()[2];
...
//         printf("Result: p1 / p0 = %f / %f = %e\n\n", p1, p0, (p0 != 0 ? p1 / p0 : -99999));
        if (p0 > 0) {
          // Only makes sense, if valid p0 available
          gSlParabelvsInvOffset->SetPoint(i, 1./p0, p1/p0);
          gSlParabelvsInvOffset->SetPointError(i, TMath::Abs(err0/p0/p0), totErrorSlope);

          gCurvParabelvsInvOffset->SetPoint(i,1/p0,p2/p0);
          gCurvParabelvsInvOffset->SetPointError(i,TMath::Abs(err0/p0/p0),totErrorCurv);
...
//             printf("Result sigma: pSigma1 / pSigma0 = %f / %f = %e\n\n", pSigma1, pSigma0, (pSigma0 != 0 ? pSigma1 / pSigma0 : -99999));
            ...
            if (pSigma0 > 0) {
              gSigmaSlopevsInvdEdxParabel->SetPoint(i, 1./p0, pSigma1/pSigma0);
              gSigmaSlopevsInvdEdxParabel->SetPointError(i, TMath::Abs(err0/p0/p0), totErrorSigma);

               // Set artificially large errors for points with positive slope vs. multiplicity
              // (positive slope makes physically no sense and is most likely due to uncertainties)
              if (pSigma1 < 0) {
                gSigmaSlopevsInvdEdxParabel->SetPointError(i, err0, 1000);
                gSigmaSlopevsInvdEdxParabel->SetPointError(i, TMath::Abs(err0/p0/p0), 1000);
              }
            }
          }
        }
    ...
    gSlParabelvsInvOffset->SetName("slopes2_Parabel_AllEta");
    gSlParabelvsInvOffset->SetTitle("Rel. slope (parabel fit) vs. 1./offset (all #eta)");
    gSlParabelvsInvOffset->SetMarkerColor(kBlack);
    gSlParabelvsInvOffset->SetLineColor(kBlack);
    gSlParabelvsInvOffset->SetMarkerStyle(20);
    gSlParabelvsInvOffset->Draw("Ap");
    gSlParabelvsInvOffset->GetHistogram()->GetXaxis()->SetTitle("1./dEdx (1./offset)");
    gSlParabelvsInvOffset->GetHistogram()->GetYaxis()->SetTitle("Multiplicity slope/offset");

    fitRes = gSlParabelvsInvOffset->Fit("pol1","EX0","same",0,0.02);

    gCurvParabelvsInvOffset->SetName("curves2_Parabel_AllEta");
    gCurvParabelvsInvOffset->SetTitle("Rel. curve (parabel fit) vs. 1./offset (all #eta)");
    gCurvParabelvsInvOffset->SetMarkerColor(kBlack);
    gCurvParabelvsInvOffset->SetLineColor(kBlack);
    gCurvParabelvsInvOffset->SetMarkerStyle(20);
    gCurvParabelvsInvOffset->Draw("Ap");
    gCurvParabelvsInvOffset->GetHistogram()->GetXaxis()->SetTitle("1./dEdx (1./offset)");
    gCurvParabelvsInvOffset->GetHistogram()->GetYaxis()->SetTitle("Multiplicity curvature/offset");

    fitRes = gCurvParabelvsInvOffset->Fit("pol1","EX0","same",0,0.02);
    ...
    TFitResultPtr fitRes2 = gSigmaSlopevsInvdEdx->Fit("pol2", "S 0 W EX0", "same", 0., 0.002); // Estimate pol2 params for subsequent fit
    TF1 fitFuncSigma("fitFuncSigma_AllEta", "TMath::Max(0, [0] + [1]*TMath::Min(x, [3]) + [2] * TMath::Power(TMath::Min(x, [3]), 2))", 0., 0.02);
    if ((Int_t)fitRes2 == 0)
    ...
    gSigmaSlopevsInvdEdxParabel->SetName("sigmaSlopes2_Parabel_AllEta");
    gSigmaSlopevsInvdEdxParabel->SetTitle("Sigma rel slope vs. 1./dEdx offset (all #eta), offset from parabel fit");
    gSigmaSlopevsInvdEdxParabel->SetMarkerColor(kRed);
    gSigmaSlopevsInvdEdxParabel->SetLineColor(kRed);
    gSigmaSlopevsInvdEdxParabel->SetMarkerStyle(20);
    gSigmaSlopevsInvdEdxParabel->Draw("Ap");
    gSigmaSlopevsInvdEdxParabel->GetHistogram()->GetXaxis()->SetTitle("1./dEdx (1./offset)");
    gSigmaSlopevsInvdEdxParabel->GetHistogram()->GetYaxis()->SetTitle("Multiplicity sigma rel. slope, offset from parabel fit");

    TFitResultPtr fitRes3 = gSigmaSlopevsInvdEdxParabel->Fit("pol2", "S 0 W EX0", "same", 0., 0.012); // Estimate pol2 params for subsequent fit
    TF1 fitFuncSigmaParabel("fitFuncSigma_Parabel_AllEta", "TMath::Max(0, [0] + [1]*TMath::Min(x, [3]) + [2] * TMath::Power(TMath::Min(x, [3]), 2))", 0., 0.2);
    if ((Int_t)fitRes3 == 0)
      fitFuncSigmaParabel.SetParameters(fitRes3->GetParams());
    fitFuncSigmaParabel.SetParameter(3, 0.012);
    fitFuncSigmaParabel.SetParLimits(3, 0.01, 0.2);

    fitFuncSigmaParabel.SetLineColor(kRed);

    gSigmaSlopevsInvdEdxParabel->Fit(&fitFuncSigmaParabel, "EX0", "same", 0., 0.02);
    ...
    gSlParabelvsInvOffset->Write();
    gCurvParabelvsInvOffset->Write();
    gSigmaSlopevsInvdEdxParabel->Write();
  ...
  for (Int_t j = 0; j < numTanThetaBins; j++) {
    ...
    for (Int_t i = 0; i < numPbins; i++) {
      ...
      TH1D* hMedian = (TH1D*)arr.At(4);
      hMedian->SetNameTitle(Form("hMedian_pBin%d_tanThetaBin_%d", i, j),Form("hMedian_pBin%d_tanThetaBin_%d", i, j));
      hMedian->SetLineColor(kRed);

       TH1D* hMeanNotGaus = (TH1D*)arr.At(5);
      hMeanNotGaus->SetNameTitle(Form("hMeannotGaus_pBin%d_tanThetaBin_%d", i, j),Form("hMeannotGaus_pBin%d_tanThetaBin_%d", i, j));
      hMeanNotGaus->SetLineColor(kGreen);

...
#include "AliTPCcalibResidualPID2.h"
#include "TChain.h"
...
ClassImp(AliTPCcalibResidualPID2)

Double_t AliTPCcalibResidualPID2::fgCutGeo = 1.;
Double_t AliTPCcalibResidualPID2::fgCutNcr = 0.85;
Double_t AliTPCcalibResidualPID2::fgCutNcl = 0.7;

//________________________________________________________________________
AliTPCcalibResidualPID2::AliTPCcalibResidualPID2()
    ...
    fTree_distance_vs(0.)
{
  // default Constructor
   /* fast compilation test
     ...
     .L /lustre/alice/akalweit/train/trunk/util/statsQA/AliTPCcalibResidualPID2.cxx++
   */
...
//________________________________________________________________________
AliTPCcalibResidualPID2::AliTPCcalibResidualPID2(const char *name)
...
//________________________________________________________________________
void AliTPCcalibResidualPID2::UserCreateOutputObjects()
{
...
void AliTPCcalibResidualPID2::UserExec(Option_t *)
{
...
//________________________________________________________________________
THnSparseF* AliTPCcalibResidualPID2::InitialisePIDQAHist(TString name, TString title, Bool_t IsPbPb)
{
...
//________________________________________________________________________
void AliTPCcalibResidualPID2::Process(AliESDEvent *const esdEvent, AliMCEvent *const mcEvent)
{
...
//________________________________________________________________________
Bool_t AliTPCcalibResidualPID2::ProcessV0Tree(TTree* tree, THnSparseF* h, const Int_t recoPass/*=4*/,
...
//________________________________________________________________________
THnSparseF* AliTPCcalibResidualPID2::ProcessV0TreeFile(TString filePathName, const Int_t recoPass/*=4*/,
                                                      const TString runList/*=""*/, const Bool_t excludeRuns/*=kFALSE*/)
{
...
//________________________________________________________________________
Int_t  AliTPCcalibResidualPID2::CompareFloat(Float_t f1, Float_t f2) const
{
...
//________________________________________________________________________
void AliTPCcalibResidualPID2::Terminate(const Option_t *)
{
...
//________________________________________________________________________
void AliTPCcalibResidualPID2::BinLogAxis(THnSparseF *h, Int_t axisNumber)
{
  //
...
void AliTPCcalibResidualPID2::CreatePlotWithOwnParameters(THnSparseF * histPidQA, const Bool_t useV0s, const Char_t * type, const Char_t * system, const Double_t* parameters, AliTPCcalibResidualPID2::FitType fitType, Float_t from, Float_t to) {
  ...
  TGraphErrors * graphProton = (TGraphErrors *) arr->FindObject("protonV0GraphForBBfit");
  TGraphErrors * graphElectron = (TGraphErrors *) arr->FindObject("electronV0plusTOFGraphForBBfit");
  TGraphErrors * graphPion = (TGraphErrors *) arr->FindObject("pionV0GraphForBBfit");
  TGraphErrors * graphDeuteron = (TGraphErrors *) arr->FindObject("deuteronTpcGraph");
  //

  TCanvas* canvDelta_2 = CreateResidualCanvas(graphAll, func);
  TCanvas* canvDelta_1 = CreateBBCanvas(arr, isMC, func);
  TCanvas * canvDelta_3 = CreateResidualCanvas_Color(graphElectron, graphPion, graphProton,graphDeuteron, func);
  return;
}

//________________________________________________________________________
Double_t* AliTPCcalibResidualPID2::ExtractResidualPID(THnSparseF * histPidQA, const Bool_t useV0s, const Char_t * outFile,
                                                     ...
                                                     AliTPCcalibResidualPID2::FitType fitType) {
  //
...
//________________________________________________________________________
TObjArray * AliTPCcalibResidualPID2::GetSeparation(THnSparseF * histPidQA, Int_t kParticle1, Int_t kParticle2) {
  ...
  TObjArray arr;
  histPart1->FitSlicesY(0,0,-1,10,"QR",&arr);
  ...
  histPart2->RebinX(4);
  histPart2->FitSlicesY(0,0,-1,10,"QR",&arr);
...
//________________________________________________________________________
TObjArray * AliTPCcalibResidualPID2::GetResidualGraphs(THnSparseF * histPidQA, const Char_t * system, Bool_t useV0s) {
  //
  ...
  Float_t nSigmaTritonPlus = 6;
  Float_t nSigmaHelium3Minus = -6;
  Float_t nSigmaHelium3Plus = 6;
  ...
  TCanvas * canvasQADeuteron = new TCanvas("canvasQADeuteron","Control canvas for residual graphs (Deuterons)",100,10,1380,800);
  canvasQADeuteron->Divide(2,1);

  TCanvas * canvasQATriton = new TCanvas("canvasQATriton","Control canvas for residual graphs (Triton)", 100,10,1380,800);
  canvasQATriton->Divide(2,1);

  TCanvas * canvasQAHelium3 = new TCanvas("canvasQAHelium3","Control canvas for residual graphs (Helium3)", 100, 10, 1380,800);
  canvasQAHelium3->Divide(2,1);
...
for (Int_t i = 1; i <= 2; i++){
  canvasQADeuteron->GetPad(i)->SetGrid(1,1);
  canvasQADeuteron->GetPad(i)->SetLogz();
  canvasQADeuteron->GetPad(i)->SetLogx();

  canvasQATriton->GetPad(i)->SetGrid(1,1);
  canvasQATriton->GetPad(i)->SetLogz();
  canvasQATriton->GetPad(i)->SetLogx();

  canvasQAHelium3->GetPad(i)->SetGrid(1,1);
  canvasQAHelium3->GetPad(i)->SetLogz();
  canvasQAHelium3->GetPad(i)->SetLogx();
}
  ...
  TObjArray arr;
  FitSlicesY(histElectron, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  FitSlicesY(histProtonTPC, heightFractionForFittingRange, cutForFitting, "QRL", &arr);
  ...
  FitSlicesY(histProtonTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  FitSlicesY(histPionTPC, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  FitSlicesY(histPionTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
...
TF1 betaSq2("betaSq","20./TMath::Power((x+0.1),1.3) + (1-TMath::Erf((x-0.35) / 2.55)) * 20",0.1,10);
  for(Int_t ix = 1; ix <= histKaonTPC->GetXaxis()->GetNbins(); ix++) {
     for(Int_t jy = 1; jy <= histKaonTPC->GetYaxis()->GetNbins(); jy++) {
       ...
       Float_t xPos = histKaonTPC->GetXaxis()->GetBinCenter(ix);
       Int_t bin = histKaonTPC->GetBin(ix,jy);
       if (yPos < betaSq2.Eval(xPos)) histKaonTPC->SetBinContent(bin,0);
     }
  }

  FitSlicesY(histKaonTPC, heightFractionForFittingRange, cutForFitting, "QRL+", &arr);
  ...
  for(Int_t ip = 0; ip < kaonTpcGraph->GetN(); ip ++) {
    Bool_t removePoint = kaonTpcGraph->GetY()[ip] < 10 || kaonTpcGraph->GetEY()[ip]/kaonTpcGraph->GetY()[ip] > 0.5
                        ...
                        || kaonTpcGraph->GetX()[ip] < 0.12 || kaonTpcGraph->GetX()[ip] > 0.40; //edited by JY #1, to avoid empty points
    if (removePoint) {
  ...
  histKaonTOF->GetXaxis()->SetRangeUser(0.3,2.0);
  /*
    for(Int_t ix = 1; ix <= histKaonTOF->GetXaxis()->GetNbins(); ix++) {
     for(Int_t jy = 1; jy <= histKaonTOF->GetYaxis()->GetNbins(); jy++) {
       Float_t yPos = histKaonTOF->GetYaxis()->GetBinCenter(jy);
       Float_t xPos = histKaonTOF->GetXaxis()->GetBinCenter(ix);
       Int_t bin = histKaonTOF->GetBin(ix,jy);
...
*/    
  ...
  for(Int_t ip = 0; ip < kaonTofGraph->GetN(); ip ++) {
    Bool_t removePoint = kaonTofGraph->GetY()[ip] < 10 || kaonTofGraph->GetEY()[ip]/kaonTofGraph->GetY()[ip] > 0.5|| kaonTofGraph->GetX()[ip] > 2.0
                         //|| kaonTofGraph->GetX()[ip] < 0.36; //TODO BEN NOW was 0.5
                         || kaonTofGraph->GetX()[ip] < 0.40; //edited by JY #2 (correlated to edit#1)
    if (removePoint) {
  ...
  histKaonTOF->SetTitle("kaons TOF");
  ...
  //TCanvas *canvasQAtof = new TCanvas();
  //canvasQAtof->Divide(2,2);
  canvasQAtof->cd(3);
  histPionTOF->Draw("colz");
  pionPointsTOF->Draw("same");
  ...
  canvasQAtof->SaveAs("canvasQAtof.pdf");

  TCanvas *hallo = new TCanvas();
  hallo->Divide(2,2);
  hallo->cd(2);
  for (Int_t i = 1; i<5; i++){
  hallo->GetPad(i)->SetLogz();
  hallo->GetPad(i)->SetLogx();
}
  histProtonTPC->Draw("colz");
  betaSq.DrawCopy("same");
  protonPointsTPC->SetMarkerStyle(20);
  ...
  hist->GetAxis(2)->SetRangeUser(0,0); // Non-V0s
  hist->GetAxis(3)->SetRangeUser(5,5); //Deuterons

  hist->GetAxis(4)->SetRangeUser(nSigmaDeuteronMinus,nSigmaDeuteronPlus); // protons --> not reliable, use PATTERN RECOGNITION
  TH2D * histDeuteronTPC = hist->Projection(1,0);
  histDeuteronTPC->SetName("histDeuteronTPC");
  histDeuteronTPC->GetXaxis()->SetRangeUser(0.15,2.0);
  histDeuteronTPC->GetYaxis()->SetRangeUser(50, 2320);

  FitSlicesY(histDeuteronTPC, heightFractionForFittingRange, cutForFitting, "QR+", &arr);
  TH1D * deuteronPointsTPC = (TH1D*) arr.At(1)->Clone();
  //
  TGraphErrors * deuteronTpcGraph = new TGraphErrors(deuteronPointsTPC);
  ...
  TH2D * histDeuteronTOF = hist->Projection(1,0);
  histDeuteronTOF->SetName("histDeuteronTOF");
  histDeuteronTOF->GetXaxis()->SetRangeUser(0.15,2.0);
  FitSlicesY(histDeuteronTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  TH1D * deuteronPointsTOF = (TH1D*) arr.At(1)->Clone();
  //
  TGraphErrors * deuteronTofGraph = new TGraphErrors(deuteronPointsTOF);
  deuteronTofGraph->SetName("deuteronTofGraph");
  //
  for(Int_t ip = 0; ip < deuteronTofGraph->GetN(); ip ++) {
    Bool_t removePoint = deuteronTofGraph->GetY()[ip] < 10 || deuteronTofGraph->GetEY()[ip]/deuteronTofGraph->GetY()[ip] > 0.02 || deuteronTofGraph->GetX()[ip] < 1.2;
                         //|| kaonTofGraph->GetX()[ip] < 0.36; //TODO BEN NOW was 0.5
      //                   || deuteronTofGraph->GetX()[ip] < 0.44; //edited by JY #2 (correlated to edit#1)
    if (removePoint) {
      deuteronTofGraph->RemovePoint(ip);
      ip--;
      continue;
    }
  }

  canvasQADeuteron->cd(1);
  histDeuteronTPC->SetTitle("deuterons");
  histDeuteronTPC->GetYaxis()->SetRangeUser(30, 2320);
  histDeuteronTPC->GetXaxis()->SetTitle(momTitle.Data());
  histDeuteronTPC->GetYaxis()->SetTitle(dEdxTitle.Data());
  histDeuteronTPC->GetXaxis()->SetMoreLogLabels(kTRUE);
  histDeuteronTPC->GetXaxis()->SetNoExponent(kTRUE);
  histDeuteronTPC->Draw("colz");
  deuteronPointsTPC->SetMarkerStyle(20);
  deuteronPointsTOF->SetMarkerStyle(24);
  deuteronPointsTOF->Draw("same");
  deuteronPointsTPC->Draw("same");
  //
  deuteronTpcGraph->SetMarkerStyle(26);
  deuteronTpcGraph->SetMarkerColor(kMagenta);
  deuteronTpcGraph->DrawClone("p");
  deuteronTofGraph->SetMarkerStyle(25);
  deuteronTofGraph->SetMarkerColor(kMagenta);
  deuteronTofGraph->DrawClone("p");


  canvasQADeuteron->cd(2);
  histDeuteronTOF->GetYaxis()->SetRangeUser(30, 1000);
  histDeuteronTOF->SetTitle("deuteronsTOF");
  histDeuteronTOF->GetXaxis()->SetTitle(momTitle.Data());
  histDeuteronTOF->GetYaxis()->SetTitle(dEdxTitle.Data());
  histDeuteronTOF->GetXaxis()->SetMoreLogLabels(kTRUE);
  histDeuteronTOF->GetXaxis()->SetNoExponent(kTRUE);
  histDeuteronTOF->Draw("colz");
  deuteronPointsTOF->Draw("same");
  deuteronPointsTPC->Draw("same");
  deuteronTpcGraph->DrawClone("p");
  deuteronTofGraph->DrawClone("p");
  ...
  hist->GetAxis(2)->SetRangeUser(0,0); // Non-V0s
  hist->GetAxis(3)->SetRangeUser(6,6); //Triton

  hist->GetAxis(4)->SetRangeUser(nSigmaTritonMinus,nSigmaTritonPlus); // protons --> not reliable, use PATTERN RECOGNITION
  TH2D * histTritonTPC = hist->Projection(1,0);
  histTritonTPC->SetName("histTritonTPC");
  histTritonTPC->GetXaxis()->SetRangeUser(0.15,2.0);
  histTritonTPC->GetYaxis()->SetRangeUser(50, 2320);

  FitSlicesY(histTritonTPC, heightFractionForFittingRange, cutForFitting, "QR+", &arr);
  TH1D * tritonPointsTPC = (TH1D*) arr.At(1)->Clone();
  //
  TGraphErrors * tritonTpcGraph = new TGraphErrors(tritonPointsTPC);
  tritonTpcGraph->SetName("tritonTpcGraph");
  for(Int_t ip = 0; ip < tritonTpcGraph->GetN(); ip ++) {
    Bool_t removePoint = tritonTpcGraph->GetY()[ip] < 10 || tritonTpcGraph->GetEY()[ip]/tritonTpcGraph->GetY()[ip] > 0.02;
                        //|| kaonTpcGraph->GetX()[ip] < 0.12 || kaonTpcGraph->GetX()[ip] > 0.319; //TODO BEN was > 0.399 - default
        //                || tritonTpcGraph->GetX()[ip] < 0.12 || tritonTpcGraph->GetX()[ip] > 0.44; //edited by JY #1, to avoid empty points
    if (removePoint) {
      tritonTpcGraph->RemovePoint(ip);
      ip--;
      continue;
    }
  }
  ...
  TH2D * histTritonTOF = hist->Projection(1,0);
  histTritonTOF->SetName("histTritonTOF");
  histTritonTOF->GetXaxis()->SetRangeUser(0.15,2.0);
  FitSlicesY(histTritonTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  TH1D * tritonPointsTOF = (TH1D*) arr.At(1)->Clone();
  //
  TGraphErrors * tritonTofGraph = new TGraphErrors(tritonPointsTOF);
  tritonTofGraph->SetName("tritonTofGraph");
  //
  for(Int_t ip = 0; ip < tritonTofGraph->GetN(); ip ++) {
    Bool_t removePoint = tritonTofGraph->GetY()[ip] < 10 || tritonTofGraph->GetEY()[ip]/tritonTofGraph->GetY()[ip] > 0.02 || tritonTofGraph->GetX()[ip] > 1.0;
                         //|| kaonTofGraph->GetX()[ip] < 0.36; //TODO BEN NOW was 0.5
      //                   || tritonTofGraph->GetX()[ip] < 0.44; //edited by JY #2 (correlated to edit#1)
    if (removePoint) {
      tritonTofGraph->RemovePoint(ip);
      ip--;
      continue;
    }
  }
  ...
  canvasQATriton->cd(1);
  histTritonTPC->SetTitle("Tritons");
  histTritonTPC->GetYaxis()->SetRangeUser(30, 2320);
  histTritonTPC->GetXaxis()->SetTitle(momTitle.Data());
  histTritonTPC->GetYaxis()->SetTitle(dEdxTitle.Data());
  histTritonTPC->GetXaxis()->SetMoreLogLabels(kTRUE);
  histTritonTPC->GetXaxis()->SetNoExponent(kTRUE);
  histTritonTPC->Draw("colz");
  tritonPointsTPC->SetMarkerStyle(20);
  tritonPointsTOF->SetMarkerStyle(24);
  tritonPointsTOF->Draw("same");
  tritonPointsTPC->Draw("same");
  //
  tritonTpcGraph->SetMarkerStyle(26);
  tritonTpcGraph->SetMarkerColor(kMagenta);
  tritonTpcGraph->DrawClone("p");
  tritonTofGraph->SetMarkerStyle(25);
  tritonTofGraph->SetMarkerColor(kMagenta);
  tritonTofGraph->DrawClone("p");


  canvasQATriton->cd(2);
  histTritonTOF->GetYaxis()->SetRangeUser(30, 1000);
  histTritonTOF->SetTitle("TritonsTOF");
  histTritonTOF->GetXaxis()->SetTitle(momTitle.Data());
  histTritonTOF->GetYaxis()->SetTitle(dEdxTitle.Data());
  histTritonTOF->GetXaxis()->SetMoreLogLabels(kTRUE);
  histTritonTOF->GetXaxis()->SetNoExponent(kTRUE);
  histTritonTOF->Draw("colz");
  tritonPointsTOF->Draw("same");
  tritonPointsTPC->Draw("same");
  tritonTpcGraph->DrawClone("p");
  tritonTofGraph->DrawClone("p");
  ...
  hist->GetAxis(2)->SetRangeUser(0,0); // Non-V0s
  hist->GetAxis(3)->SetRangeUser(7,7); // Helium3

  hist->GetAxis(4)->SetRangeUser(nSigmaHelium3Minus,nSigmaHelium3Plus); // protons --> not reliable, use PATTERN RECOGNITION
  TH2D * histHelium3TPC = hist->Projection(1,0);
  histHelium3TPC->SetName("histHelium3TPC");
  histHelium3TPC->GetXaxis()->SetRangeUser(0.15,2.0);
  histHelium3TPC->GetYaxis()->SetRangeUser(50, 2320);

  FitSlicesY(histHelium3TPC, heightFractionForFittingRange, cutForFitting, "QR+", &arr);
  TH1D * helium3PointsTPC = (TH1D*) arr.At(1)->Clone();
  //
  TGraphErrors * helium3TpcGraph = new TGraphErrors(helium3PointsTPC);
  helium3TpcGraph->SetName("helium3TpcGraph");
  for(Int_t ip = 0; ip < helium3TpcGraph->GetN(); ip ++) {
    Bool_t removePoint = helium3TpcGraph->GetY()[ip] < 10 || helium3TpcGraph->GetEY()[ip]/helium3TpcGraph->GetY()[ip] > 0.02;
                        //|| kaonTpcGraph->GetX()[ip] < 0.12 || kaonTpcGraph->GetX()[ip] > 0.319; //TODO BEN was > 0.399 - default
        //                || tritonTpcGraph->GetX()[ip] < 0.12 || tritonTpcGraph->GetX()[ip] > 0.44; //edited by JY #1, to avoid empty points
    if (removePoint) {
      helium3TpcGraph->RemovePoint(ip);
      ip--;
      continue;
    }
  }
  ...
  TH2D * histHelium3TOF = hist->Projection(1,0);
  histHelium3TOF->SetName("histHelium3TOF");
  histHelium3TOF->GetXaxis()->SetRangeUser(0.15,2.0);
  FitSlicesY(histHelium3TOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  TH1D * helium3PointsTOF = (TH1D*) arr.At(1)->Clone();
  //
  TGraphErrors * helium3TofGraph = new TGraphErrors(helium3PointsTOF);
  helium3TofGraph->SetName("helium3TofGraph");
  //
  for(Int_t ip = 0; ip < helium3TofGraph->GetN(); ip ++) {
    Bool_t removePoint = helium3TofGraph->GetY()[ip] < 10 || helium3TofGraph->GetEY()[ip]/helium3TofGraph->GetY()[ip] > 0.02 || helium3TofGraph->GetX()[ip] > 1.0;
                         //|| kaonTofGraph->GetX()[ip] < 0.36; //TODO BEN NOW was 0.5
      //                   || tritonTofGraph->GetX()[ip] < 0.44; //edited by JY #2 (correlated to edit#1)
    if (removePoint) {
      helium3TofGraph->RemovePoint(ip);
      ip--;
      continue;
    }
  }

  canvasQAHelium3->cd(1);
  histHelium3TPC->SetTitle("Helium3s");
  histHelium3TPC->GetYaxis()->SetRangeUser(30, 2320);
  histHelium3TPC->GetXaxis()->SetTitle(momTitle.Data());
  histHelium3TPC->GetYaxis()->SetTitle(dEdxTitle.Data());
  histHelium3TPC->GetXaxis()->SetMoreLogLabels(kTRUE);
  histHelium3TPC->GetXaxis()->SetNoExponent(kTRUE);
  histHelium3TPC->Draw("colz");
  helium3PointsTPC->SetMarkerStyle(20);
  helium3PointsTOF->SetMarkerStyle(24);
  helium3PointsTOF->Draw("same");
  helium3PointsTPC->Draw("same");
  //
  helium3TpcGraph->SetMarkerStyle(26);
  helium3TpcGraph->SetMarkerColor(kMagenta);
  helium3TpcGraph->DrawClone("p");
  helium3TofGraph->SetMarkerStyle(25);
  helium3TofGraph->SetMarkerColor(kMagenta);
  helium3TofGraph->DrawClone("p");


  canvasQAHelium3->cd(2);
  histHelium3TOF->GetYaxis()->SetRangeUser(30, 1000);
  histHelium3TOF->SetTitle("Helium3sTOF");
  histHelium3TOF->GetXaxis()->SetTitle(momTitle.Data());
  histHelium3TOF->GetYaxis()->SetTitle(dEdxTitle.Data());
  histHelium3TOF->GetXaxis()->SetMoreLogLabels(kTRUE);
  histHelium3TOF->GetXaxis()->SetNoExponent(kTRUE);
  histHelium3TOF->Draw("colz");
  helium3PointsTOF->Draw("same");
  helium3PointsTPC->Draw("same");
  helium3TpcGraph->DrawClone("p");
  helium3TofGraph->DrawClone("p");
  ...
  TCanvas *c_Test2 = new TCanvas();
  histHelium3TOF->Draw("colz");
  c_Test2->SaveAs("Test2.pdf");
  ...
  FitSlicesY(histElectronV0, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  FitSlicesY(histPionV0, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  histProtonV0->GetYaxis()->SetRangeUser(0,1100);
  ...
  // PATTERN RECOGNITION
  Int_t correctionUpToBin = histProtonV0->GetXaxis()->FindBin(0.8);
  ...
  */
  ...
  FitSlicesY(histProtonV0, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  FitSlicesY(histElectronV0plusTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  FitSlicesY(histPionV0plusTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  histProtonV0plusTOF->GetYaxis()->SetRangeUser(0,300);
  ...
  FitSlicesY(histProtonV0plusTOF, heightFractionForFittingRange, cutForFitting, "QR", &arr);
  ...
  for(Int_t ip = 0; ip < electronV0plusTOFGraphForBBfit->GetN(); ip ++) {
    ...
    Bool_t removePoint = electronV0plusTOFGraphForBBfit->GetX()[ip] < 0.75 || electronV0plusTOFGraphForBBfit->GetX()[ip] > 4.0; //edited by JY #3 (cutoff below 1500 and above 8000)
  ...
  deuteronTpcGraph->SetMarkerStyle(22);
  deuteronTofGraph->SetMarkerStyle(21);
  deuteronTpcGraph->SetMarkerColor(5);
  deuteronTofGraph->SetMarkerColor(5);

  for(Int_t ip = 0; ip < deuteronTofGraph->GetN(); ip ++) {
    deuteronTofGraph->GetX()[ip] /= AliPID::ParticleMass(AliPID::kDeuteron);
    deuteronTofGraph->GetEX()[ip] /= AliPID::ParticleMass(AliPID::kDeuteron);
  }
  for(Int_t ip = 0; ip < deuteronTpcGraph->GetN(); ip ++) {
    deuteronTpcGraph->GetX()[ip] /= AliPID::ParticleMass(AliPID::kDeuteron);
    deuteronTpcGraph->GetEX()[ip] /= AliPID::ParticleMass(AliPID::kDeuteron);
  }

  tritonTpcGraph->SetMarkerStyle(22);
  tritonTofGraph->SetMarkerStyle(21);
  tritonTpcGraph->SetMarkerColor(6);
  tritonTofGraph->SetMarkerColor(6);

  for(Int_t ip = 0; ip < tritonTofGraph->GetN(); ip ++) {
    tritonTofGraph->GetX()[ip] /= AliPID::ParticleMass(AliPID::kTriton);
    tritonTofGraph->GetEX()[ip] /= AliPID::ParticleMass(AliPID::kTriton);
  }
  for(Int_t ip = 0; ip < tritonTpcGraph->GetN(); ip ++) {
    tritonTpcGraph->GetX()[ip] /= AliPID::ParticleMass(AliPID::kTriton);
    tritonTpcGraph->GetEX()[ip] /= AliPID::ParticleMass(AliPID::kTriton);
  }

  helium3TpcGraph->SetMarkerStyle(22);
  helium3TofGraph->SetMarkerStyle(21);
  helium3TpcGraph->SetMarkerColor(7);
  helium3TofGraph->SetMarkerColor(7);

  for(Int_t ip = 0; ip < helium3TofGraph->GetN(); ip ++) {
    helium3TofGraph->GetX()[ip] /= AliPID::ParticleMass(AliPID::kHe3);
    helium3TofGraph->GetEX()[ip] /= AliPID::ParticleMass(AliPID::kHe3);
  }
  for(Int_t ip = 0; ip < helium3TpcGraph->GetN(); ip ++) {
    helium3TpcGraph->GetX()[ip] /= AliPID::ParticleMass(AliPID::kHe3);
    helium3TpcGraph->GetEX()[ip] /= AliPID::ParticleMass(AliPID::kHe3);
  }
  ...
  arrGraphs->Add(electronV0plusTOFGraphForBBfit);
  ...
  arrGraphs->Add(deuteronTpcGraph);
  arrGraphs->Add(deuteronTofGraph);
  arrGraphs->Add(tritonTpcGraph);
  arrGraphs->Add(tritonTofGraph);
  arrGraphs->Add(helium3TpcGraph);
  arrGraphs->Add(helium3TofGraph);
  ...
  canvasQAv0->SaveAs("splines_QA_ResidualGraphsV0.pdf");
  canvasQAv0plusTOF->SaveAs("splines_QA_ResidualGraphsV0plusTOF.pdf");
  canvasQADeuteron->SaveAs("splines_QA_deuterons.pdf");
  canvasQADeuteron->SaveAs("splines_QA_deuterons.root");
  canvasQATriton->SaveAs("splines_QA_tritons.pdf");
  canvasQATriton->SaveAs("splines_QA_tritons.root");
  canvasQAHelium3->SaveAs("splines_QA_helium3s.pdf");
  canvasQAHelium3->SaveAs("splines_QA_helium3s.pdf");
...
//________________________________________________________________________
TObjArray * AliTPCcalibResidualPID2::GetResidualGraphsMC(THnSparseF * histPidQA, const Char_t * /*system*/) {
  //
  ...
  TObjArray arr;
  FitSlicesY(histElectron, heightFractionForFittingRange, cutForFitting, "QRL", &arr);
  TH1D * electronPoints = (TH1D *) arr.At(1)->Clone();
  //
  TGraphErrors * electronGraph = new TGraphErrors(electronPoints);
  ...
  FitSlicesY(histProton, heightFractionForFittingRange, cutForFitting, "QRL", &arr);
  ...
  FitSlicesY(histPion, heightFractionForFittingRange, cutForFitting, "QRL", &arr);
  ...
  FitSlicesY(histKaon, heightFractionForFittingRange, cutForFitting, "QRL", &arr);
...
//________________________________________________________________________
TObjArray * AliTPCcalibResidualPID2::GetResponseFunctions(TF1* parametrisation, TObjArray* inputGraphs, const Char_t * type, const Char_t * period, const Char_t * pass, const Char_t * system, const Char_t* dedxtype) {
  //
  //
  ...
  */
  ...
  TString fitFuncString2 = "[0] + [1] / x + [2] / (x**2) + [3] / (x**3) + [4] / (x**4) + [5] / (x**5) + [6] * x";
  TString fitFuncString_pol6 = "pol6";


  TGraphErrors * graphDeuteronTPC = (TGraphErrors *) inputGraphs->FindObject("deuteronTpcGraph");
  TGraphErrors *  graphDeuteronTPCfinal = new TGraphErrors(*graphDeuteronTPC);
  graphDeuteronTPCfinal->SetName("graphDeuteronTPCfinal");
  for(Int_t ip = 0; ip < graphDeuteronTPC->GetN(); ip ++) {
    graphDeuteronTPC->GetY()[ip] -= funcBB->Eval(graphDeuteronTPC->GetX()[ip]);
    graphDeuteronTPC->GetY()[ip] /= funcBB->Eval(graphDeuteronTPC->GetX()[ip]);
    graphDeuteronTPC->GetEY()[ip] /= funcBB->Eval(graphDeuteronTPC->GetX()[ip]);;
  }
  TGraphErrors * graphDeuteronTOF = (TGraphErrors *) inputGraphs->FindObject("deuteronTofGraph");
  TGraphErrors *  graphDeuteronTOFfinal = new TGraphErrors(*graphDeuteronTOF);
  graphDeuteronTOFfinal->SetName("graphDeuteronTOFfinal");
  for(Int_t ip = 0; ip < graphDeuteronTOF->GetN(); ip ++) {
    graphDeuteronTOF->GetY()[ip] -= funcBB->Eval(graphDeuteronTOF->GetX()[ip]);
    graphDeuteronTOF->GetY()[ip] /= funcBB->Eval(graphDeuteronTOF->GetX()[ip]);
    graphDeuteronTOF->GetEY()[ip] /= funcBB->Eval(graphDeuteronTOF->GetX()[ip]);;
  }

  canvasQA[4]->cd();
  graphDeuteronTPC->GetXaxis()->SetTitle("#beta#gamma");
  graphDeuteronTPC->GetYaxis()->SetRangeUser(-0.15,0.15);
  graphDeuteronTPC->GetXaxis()->SetMoreLogLabels(kTRUE);
  graphDeuteronTPC->GetXaxis()->SetNoExponent(kTRUE);
  graphDeuteronTPC->GetYaxis()->SetLabelSize(0.04);
  graphDeuteronTPC->GetYaxis()->SetTitleOffset(0.85);
  graphDeuteronTPC->GetXaxis()->SetTitleOffset(1.0);
  graphDeuteronTPC->GetYaxis()->SetTitle("(data - fit) / fit");
  graphDeuteronTPC->Draw("ap");


  TGraphErrors * graphTritonTPC = (TGraphErrors *) inputGraphs->FindObject("tritonTpcGraph");
  TGraphErrors *  graphTritonTPCfinal = new TGraphErrors(*graphTritonTPC);
  graphTritonTPCfinal->SetName("graphTritonTPCfinal");
  for(Int_t ip = 0; ip < graphTritonTPC->GetN(); ip ++) {
    graphTritonTPC->GetY()[ip] -= funcBB->Eval(graphTritonTPC->GetX()[ip]);
    graphTritonTPC->GetY()[ip] /= funcBB->Eval(graphTritonTPC->GetX()[ip]);
    graphTritonTPC->GetEY()[ip] /= funcBB->Eval(graphTritonTPC->GetX()[ip]);;
  }
  canvasQA[5]->cd();
  graphTritonTPC->GetXaxis()->SetTitle("#beta#gamma");
  graphTritonTPC->GetYaxis()->SetRangeUser(-0.2,0.2);
  graphTritonTPC->GetXaxis()->SetMoreLogLabels(kTRUE);
  graphTritonTPC->GetXaxis()->SetNoExponent(kTRUE);
  graphTritonTPC->GetYaxis()->SetLabelSize(0.04);
  graphTritonTPC->GetYaxis()->SetTitleOffset(0.85);
  graphTritonTPC->GetXaxis()->SetTitleOffset(1.0);
  graphTritonTPC->GetYaxis()->SetTitle("(data - fit) / fit");
  //graphTritonTPC->Draw("ap");

  TGraphErrors * graphHelium3TPC = (TGraphErrors *) inputGraphs->FindObject("helium3TpcGraph");
  TGraphErrors *  graphHelium3TPCfinal = new TGraphErrors(*graphHelium3TPC);
  graphHelium3TPCfinal->SetName("graphHelium3TPCfinal");
  for(Int_t ip = 0; ip < graphHelium3TPC->GetN(); ip ++) {
    graphHelium3TPC->GetY()[ip] -= funcBB->Eval(graphHelium3TPC->GetX()[ip]);
    graphHelium3TPC->GetY()[ip] /= funcBB->Eval(graphHelium3TPC->GetX()[ip]);
    graphHelium3TPC->GetEY()[ip] /= funcBB->Eval(graphHelium3TPC->GetX()[ip]);;
  }
  canvasQA[6]->cd();
  graphHelium3TPC->GetXaxis()->SetTitle("#beta#gamma");
  graphHelium3TPC->GetYaxis()->SetRangeUser(-0.2,0.2);
  graphHelium3TPC->GetXaxis()->SetMoreLogLabels(kTRUE);
  graphHelium3TPC->GetXaxis()->SetNoExponent(kTRUE);
  graphHelium3TPC->GetYaxis()->SetLabelSize(0.04);
  graphHelium3TPC->GetYaxis()->SetTitleOffset(0.85);
  graphHelium3TPC->GetXaxis()->SetTitleOffset(1.0);
  graphHelium3TPC->GetYaxis()->SetTitle("(data - fit) / fit");
  graphHelium3TPC->Draw("ap");
  ...
  TF1 * funcCorrProton = new TF1("funcCorrProton", "pol8", TMath::Max(graphProtonTPC->GetX()[0], 0.18),0.6); // TODO BEN was 0.18 - 0.85 //nschmidt2016 was ...,1.0); //was then 2.0 
 ...
 // graphKaonTPCfinal->RemovePoint(14);
  //graphKaonTPCfinal->RemovePoint(15);
  //graphKaonTPCfinal->RemovePoint(15);
  //graphKaonTPCfinal->RemovePoint(14);
  canvasQA[1]->cd();
  //canvasQA->cd(2);
  ...
  graphKaonTPC->GetYaxis()->SetRangeUser(-0.1,0.1);
...
//for(Int_t ip = 0; ip < graphKaonTPC->GetN(); ip ++) {
//cout << ip << graphKaonTPC->GetX() [ip] << endl;
//}
  ...
  if (!kaonGraphHasDataPoints) {
    ...
    funcCorrKaon = new TF1("funcCorrKaon", "0", 0.1, 3);
  }
  else {
    if (isMC) {
...
*/
      ...
      funcCorrKaon = new TF1("funcCorrKaon", "pol6", 0.25, 2.0);
      graphKaonTPC->Fit(funcCorrKaon, "QREX0M","",0.25,2.0);
  ...
  TF1 * funcCorrPion = new TF1("funcCorrPion", fitFuncString.Data(), (0.2 / AliPID::ParticleMass(AliPID::kPion)), 24.5); //edited by JY #4, to optimize LHC18r,q(PbPb), works well with pp
  ...
  TF1 * funcCorrElectron = new TF1("funcCorrElectron", fitFuncString.Data(), (0.2 / AliPID::ParticleMass(AliPID::kElectron)), 4000);// edited by JY #5, to get better fit, to reduce fluctuation in pp, PbPb
  ...
  for(Int_t kk = 0; kk < nBins; kk++) {
    ...
    yDeuteron[kk] = funcBB->Eval(xBetaGamma[kk])/50.;
  ...
  TGraph * graphDeuteron = new TGraph(nBins, xBetaGamma, yDeuteron);
  ...
  TSpline3 * splineDeuteron = new TSpline3("splineDeuteron",graphDeuteron);
  ...
  arrResponse->AddLast(splineDeuteron);
...
//  TGraphErrors *  graphDeuteronTPCfinal = new TGraphErrors(*graphDeuteronTPC);
  canvasQA[5]->cd();
  graphDeuteronTPCfinal->SetName("graphDeuteronTPCfinal");
  for(Int_t ip = 0; ip < graphDeuteronTPCfinal->GetN(); ip ++) {
    graphDeuteronTPCfinal->GetY()[ip] -= 50. * splineProton->Eval(graphDeuteronTPCfinal->GetX()[ip]);
    graphDeuteronTPCfinal->GetY()[ip] /= 50. * splineProton->Eval(graphDeuteronTPCfinal->GetX()[ip]);
    graphDeuteronTPCfinal->GetEY()[ip] /= 50. * splineProton->Eval(graphDeuteronTPCfinal->GetX()[ip]);;
  }
  graphDeuteronTPCfinal->GetXaxis()->SetTitle("#beta#gamma");
  graphDeuteronTPCfinal->GetYaxis()->SetRangeUser(-0.2,0.2);
  graphDeuteronTPCfinal->GetXaxis()->SetMoreLogLabels(kTRUE);
  graphDeuteronTPCfinal->GetXaxis()->SetNoExponent(kTRUE);
  graphDeuteronTPCfinal->GetYaxis()->SetLabelSize(0.04);
  graphDeuteronTPCfinal->GetYaxis()->SetTitleOffset(0.85);
  graphDeuteronTPCfinal->GetXaxis()->SetTitleOffset(1.0);
  graphDeuteronTPCfinal->GetYaxis()->SetTitle("(data - fit) / fit");
  graphDeuteronTPCfinal->Draw("ap");
...
TF1* AliTPCcalibResidualPID2::SetUpFitFunction(const Double_t* initialParameters, AliTPCcalibResidualPID2::FitType fitType, Float_t from, Float_t to, Bool_t isPPb, Bool_t isMC, Double_t* parametersBBForward) {
  ...
  if (fitType == AliTPCcalibResidualPID2::kSaturatedLund) {
  ...
  else if (fitType == AliTPCcalibResidualPID2::kLund) {
  ...
  else if (fitType == AliTPCcalibResidualPID2::kAleph) {
  ...
  else if (fitType == AliTPCcalibResidualPID2::kAlephExternal) {
...
void AliTPCcalibResidualPID2::SetUpInputGraph(TGraphErrors* graphAll, Bool_t isMC, Bool_t useV0s) {
...
TCanvas* AliTPCcalibResidualPID2::CreateBBCanvas(TObjArray* inputGraphs, Bool_t isMC, TF1* func) {
  ...
  TH1F *hBBdummy=new TH1F("hBBdummy","Bethe-Bloch fit;#beta#gamma;#LTdE/dx#GT (arb. unit)",100,.2,1e4);
  hBBdummy->SetMinimum(45);
  hBBdummy->SetMaximum(120); //2520 for deut
  ...
  TLegend *leg=new TLegend(.4,0.4,0.56,isMC?.6:0.8);
  leg->SetFillColor(10);
  leg->SetBorderSize(1);
  ...
  if (isMC) {
    ...
    gr=(TGraph*)inputGraphs->FindObject("deuteronTpcGraph");
    gr->SetMarkerColor(28);
    gr->SetMarkerStyle(30);
    gr->Draw("p");
    leg->AddEntry(gr,"d (TPC)","p");
    gr=(TGraph*)inputGraphs->FindObject("deuteronTofGraph");
    gr->SetMarkerColor(28);
    gr->SetMarkerStyle(32);
    gr->Draw("p");
    leg->AddEntry(gr,"d (TOF)","p");
    /*
    gr=(TGraph*)inputGraphs->FindObject("tritonTpcGraph");
    gr->SetMarkerColor(51);
    gr->SetMarkerStyle(32);
    gr->Draw("p");
    leg->AddEntry(gr,"t (TPC)","p");
    gr=(TGraph*)inputGraphs->FindObject("tritonTofGraph");
    gr->SetMarkerColor(51);
    gr->SetMarkerStyle(42);
    gr->Draw("p");
    leg->AddEntry(gr,"t (TOF)","p");
    gr=(TGraph*)inputGraphs->FindObject("helium3TpcGraph");
    gr->SetMarkerColor(93);
    gr->SetMarkerStyle(32);
    gr->Draw("p");
    leg->AddEntry(gr,"He3 (TPC)","p");
    gr=(TGraph*)inputGraphs->FindObject("helium3TofGraph");
    gr->SetMarkerColor(93);
    gr->SetMarkerStyle(42);
    gr->Draw("p");
    leg->AddEntry(gr,"He3 (TOF)","p");
    */
  }
  ...
  //latex.SetTextAlign(13);  //align at to
  //latex.DrawLatexNDC(0.6,0.85,"#bf{ALICE}");
  //latex.DrawLatexNDC(.52,.78,"#bf{Pb-Pb}, #bf{#sqrt{#it{s_{NN}}} = 5.02 TeV}");
  //latex.DrawLatexNDC(.52,.68,"#bf{LHC18q pass3}");

  return canvDelta_1;
}


TCanvas* AliTPCcalibResidualPID2::CreateResidualCanvas(TGraphErrors * graphAll, TF1* func) {
  ...
  TH1F *hBBresdummy=new TH1F("hBBresdummy","residuals of BB fit;#beta#gamma;(data-fit)/fit",100,.2,1e4);
...
TCanvas* AliTPCcalibResidualPID2::CreateResidualCanvas_Color(TGraphErrors * graphkaon,TGraphErrors *graphpion, TGraphErrors *graphproton, TGraphErrors *graphDeuteron, TF1* func) {

  TGraphErrors * graphDelta_proton = new TGraphErrors(*graphproton);
  graphDelta_proton->SetName("graphDelta_proton");

  for(Int_t ip = 0; ip < graphDelta_proton->GetN(); ip++) {
    graphDelta_proton->GetY()[ip] -= func->Eval(graphDelta_proton->GetX()[ip]);
    graphDelta_proton->GetY()[ip] /= func->Eval(graphDelta_proton->GetX()[ip]);
    graphDelta_proton->GetEY()[ip] /= func->Eval(graphDelta_proton->GetX()[ip]);
  }

    TGraphErrors * graphDelta_kaon = new TGraphErrors(*graphkaon);
  graphDelta_kaon->SetName("graphDelta_kaon");

  for(Int_t ip = 0; ip < graphDelta_kaon->GetN(); ip++) {
    graphDelta_kaon->GetY()[ip] -= func->Eval(graphDelta_kaon->GetX()[ip]);
    graphDelta_kaon->GetY()[ip] /= func->Eval(graphDelta_kaon->GetX()[ip]);
    graphDelta_kaon->GetEY()[ip] /= func->Eval(graphDelta_kaon->GetX()[ip]);
  }

    TGraphErrors * graphDelta_pion = new TGraphErrors(*graphpion);
  graphDelta_pion->SetName("graphDelta_pion");

  for(Int_t ip = 0; ip < graphDelta_pion->GetN(); ip++) {
    graphDelta_pion->GetY()[ip] -= func->Eval(graphDelta_pion->GetX()[ip]);
    graphDelta_pion->GetY()[ip] /= func->Eval(graphDelta_pion->GetX()[ip]);
    graphDelta_pion->GetEY()[ip] /= func->Eval(graphDelta_pion->GetX()[ip]);
  }

  TGraphErrors * graphDelta_deut = new TGraphErrors(*graphDeuteron);
  graphDelta_deut->SetName("graphDelta__deut");

  for(Int_t ip = 0; ip < graphDelta_deut->GetN(); ip++) {
    graphDelta_deut->GetY()[ip] -= func->Eval(graphDelta_deut->GetX()[ip]);
    graphDelta_deut->GetY()[ip] /= func->Eval(graphDelta_deut->GetX()[ip]);
    graphDelta_deut->GetEY()[ip] /= func->Eval(graphDelta_deut->GetX()[ip]);
  }



  TCanvas * canvDelta_3 = new TCanvas("canvDelta_3","control histogram for Bethe-Bloch fit 2",100,10,1380,800);
  canvDelta_3->SetGrid(1, 1);
  canvDelta_3->SetLogx();
  canvDelta_3->cd();

  TH1F *hBBresdummy2=new TH1F("hBBresdummy2","residuals of BB fit;#beta#gamma;(data-fit)/fit",100,.2,1e4);
  hBBresdummy2->SetMinimum(-0.04);
  hBBresdummy2->SetMaximum(0.04);
  hBBresdummy2->GetXaxis()->SetTitleOffset(1.1);
  hBBresdummy2->GetYaxis()->SetTitleOffset(0.8);
  hBBresdummy2->SetStats(kFALSE);
  hBBresdummy2->Draw();



  graphDelta_proton->SetTitle("residuals of BB fit to multi-graph");
  graphDelta_proton->GetXaxis()->SetTitle("#beta#gamma");
  graphDelta_proton->GetYaxis()->SetTitle("(data - fit) / fit");
  graphDelta_proton->SetMarkerStyle(22);
  graphDelta_proton->SetMarkerColor(2);
  graphDelta_kaon->SetMarkerColor(1);
  graphDelta_pion->SetMarkerColor(4);
  graphDelta_deut->SetMarkerColor(5);
  graphDelta_proton->Draw("p");
  graphDelta_kaon->Draw("psame");
  graphDelta_pion->Draw("psame");
  graphDelta_deut->Draw("psame");
...
//________________________________________________________________________
TF1* AliTPCcalibResidualPID2::FitBB(TObjArray* inputGraphs, Bool_t isMC, Bool_t isPPb, const Bool_t useV0s,
                                   const Double_t * initialParameters, AliTPCcalibResidualPID2::FitType fitType) {
  //
  // Fit Bethe-Bloch parametrisation to data points (inputGraphs)
  //
  TGraphErrors * graphAll = (TGraphErrors *) inputGraphs->FindObject("beamDataPoints");
  TGraphErrors * graphProton = (TGraphErrors *) inputGraphs->FindObject("protonV0GraphForBBfit");
  TGraphErrors * graphElectron = (TGraphErrors *) inputGraphs->FindObject("electronV0plusTOFGraphForBBfit");
  TGraphErrors * graphPion = (TGraphErrors *) inputGraphs->FindObject("pionV0GraphForBBfit");
  TGraphErrors * graphDeuteron = (TGraphErrors *) inputGraphs->FindObject("deuteronTpcGraph");
  //TCanvas * canvDelta_3 = CreateResidualCanvas_Color(graphElectron, graphPion, graphProton, funcBB);
  Float_t from = 0.1; //TODO ADJUST -> Very important
...
gStyle->SetOptFit(1110);
  ...
  TCanvas * canvDelta_3 = CreateResidualCanvas_Color(graphElectron, graphPion, graphProton, graphDeuteron, funcBB);
  ...
  canvDelta_3->Write();
  canvDelta_3->SaveAs(TString::Format("betheblochresidual_color_%s.pdf",fitTypeName.Data()).Data());
...
//________________________________________________________________________
Double_t AliTPCcalibResidualPID2::Lund(Double_t* xx, Double_t* par)
{
  // bg is beta*gamma
...
//________________________________________________________________________
Double_t AliTPCcalibResidualPID2::SaturatedLund(Double_t* xx, Double_t* par)
{
  const Double_t qq = Lund(xx, par);
  return qq * TMath::Exp(par[5] / qq);
}

Double_t AliTPCcalibResidualPID2::Aleph(Double_t* xx, Double_t* par) {
  const Double_t bg = xx[0];
...
//________________________________________________________________________
void AliTPCcalibResidualPID2::FitSlicesY(TH2 *hist, Double_t heightFractionForRange, Int_t cutThreshold, TString fitOption, TObjArray *arr)
{
  ...
  */
  ...
  for (Int_t ibin=axis->GetFirst(); ibin<=axis->GetLast(); ++ibin){
    ...
    h->GetXaxis()->SetRange(h->FindFirstBinAbove(0)-20,h->FindLastBinAbove(0)+20);
    h->SetTitle(Form("%s from %f GeV/c to %f GeV/c",name,momentum_low,momentum_up));
...
Bool_t AliTPCcalibResidualPID2::GetVertexIsOk(AliVEvent* event) const
{
  AliAODEvent* aod = 0x0;
  AliESDEvent* esd = 0x0;
...
//______________________________________________________________________________
void AliTPCcalibResidualPID2::FillV0PIDlist(AliESDEvent* event)
{
  //
...
//______________________________________________________________________________
void AliTPCcalibResidualPID2::ClearV0PIDlist()
{
  //
...
//______________________________________________________________________________
Char_t AliTPCcalibResidualPID2::GetV0tag(Int_t trackIndex) const
{
  //
...
//______________________________________________________________________________
Int_t AliTPCcalibResidualPID2::GetV0motherIndex(Int_t trackIndex) const
{
  //
...
//______________________________________________________________________________
Int_t AliTPCcalibResidualPID2::GetV0motherPDG(Int_t trackIndex) const
{
  //
...
//______________________________________________________________________________
Int_t AliTPCcalibResidualPID2::MergeGraphErrors(TGraphErrors* mergedGraph, TCollection* li)
{
...
//________________________________________________________________________
Bool_t AliTPCcalibResidualPID2::TPCCutMIGeo(const AliVTrack* track, const AliVEvent* evt, TTreeStream* streamer)
{
  //
  // TPC Cut MIGeo
  //
...
//________________________________________________________________________
void AliTPCcalibResidualPID2::SetAxisNamesFromTitle(const THnSparseF *h)
{
...
//________________________________________________________________________
TString AliTPCcalibResidualPID2::GetStringFitType(Int_t fitType) {
  if (fitType == AliTPCcalibResidualPID2::kLund)
    return "Lund";
  else if (fitType == AliTPCcalibResidualPID2::kSaturatedLund)
    return "Saturated Lund";
  else if (fitType == AliTPCcalibResidualPID2::kAleph)
    return "ALEPH";
  else if (fitType == AliTPCcalibResidualPID2::kAlephWithAdditionalParam)

...
#include "/home/michael/Documents/Splines/PIDcalib_software/correctShapeEtaTree.C"
...
Int_t checkPullTree(TString pathTree,  TString pathNameThetaMap, TString pathNameSigmaMap,
                    TString mapSuffix, const TString collTypeString /*0: pp, 1: pPb, 2: PbPb*/, const Bool_t plotPull = kTRUE,
                    const Double_t downScaleFactor = 1,
                    TString pathNameSplinesFile = "", TString prSplinesName = "",
                    TString fileNameTree = "bhess_PIDetaTree.root", TString treeName = "fTree", const TString multCorrFactor = "", TString MultSeparationString = "")
{

  Int_t collType = 0;

  if (collTypeString.EqualTo("PP",TString::kIgnoreCase))
    collType = 0;
  else if (collTypeString.EqualTo("PPB",TString::kIgnoreCase) || collTypeString.EqualTo("PBP", TString::kIgnoreCase))
    collType = 1;
  else if (collTypeString.EqualTo("PBPB",TString::kIgnoreCase))
    collType = 2;
  else {
    printf("No known collision type recognised, assuming pp\n");
    collType = 0;
  }
  ...
  TSpline3** splinesPr = 0x0;
  TArrayI* multArrayBins = 0x0;
  Int_t nOfMultBins = 1;

  if (recalculateExpecteddEdx) {
    gROOT->LoadMacro("/home/michael/Documents/Splines/PIDcalib_software/correctShapeEtaTree.C+");
    std::cout << "Loading splines to recalculate expected dEdx!" << std::endl << std::endl;

    TObjArray* multArray = MultSeparationString.Tokenize(":");
    nOfMultBins = TMath::FloorNint((multArray->GetEntries()+0.1)/2.0);
    if (nOfMultBins < 1) {
      splinesPr = new TSpline3*[1];
      //Load the splines
      splinesPr[0] = getSplineFromFile(pathNameSplinesFile, prSplinesName);
    }
    else {
      splinesPr = new TSpline3*[nOfMultBins];
      multArrayBins = new TArrayI(2*nOfMultBins);
      pathNameSplinesFile.ReplaceAll(".root","");
      for (Int_t i=0;i<nOfMultBins;++i) {
        Int_t lowMult = ((TObjString*)(multArray->At(2*i)))->String().Atoi();
        Int_t highMult = ((TObjString*)(multArray->At(2*i+1)))->String().Atoi();
        multArrayBins->AddAt(lowMult, 2*i);
        multArrayBins->AddAt(highMult, 2*i+1);
        splinesPr[i] = getSplineFromFile(TString::Format("%s_%d_%d.root",pathNameSplinesFile.Data(), lowMult, highMult), prSplinesName);
        if (!splinesPr[i]) {
          std::cout << "No Splines found for File " << TString::Format("%s_%d_%d.root",pathNameSplinesFile.Data(), lowMult, highMult) << std::endl;
          break;
        }
      }
      if (!splinesPr[0]) {
        splinesPr[0] = getSplineFromFile(pathNameSplinesFile.Append(".root"), prSplinesName);
        nOfMultBins = 1;
      }
      for (Int_t i=1;i<multArrayBins->GetSize()-1;i=i+2) {
        Int_t mid = TMath::FloorNint((multArrayBins->At(i) + multArrayBins->At(i+1))/2.0);
        multArrayBins->AddAt(mid,i);
        multArrayBins->AddAt(mid,i+1);
      }
      multArrayBins->AddAt(1000000,multArrayBins->GetSize()-1);
    }
  }
  else
  ...
  Double_t tpcSignalN = 0; // Number of clusters used for dEdx
  Double_t  pidType = 0;
  Double_t    fMultiplicity = 0;
  //Double_t phiPrime = 0;
  ...
  */
  ...
  TH2D** hPullAdditionalCorrMult = new TH2D*[nMultHistos];

  for (Int_t i=0;i<nMultHistos;i++) {
    hPullAdditionalCorrMult[i] = new TH2D(Form("hPullAdditionalCorrMult_%d",i),Form("Pull vs. p_{TPC} for %d <= Multiplicity < %d with additional dEdx correction w.r.t. tan(#Theta);p_{TPC} (GeV/c);Pull",i * multStepSize, (i+1) * multStepSize), nPbinsForMap, binsPforMap, plotPull ? 120 : 240, plotPull ? -6 : -0.6, plotPull ? 6 : 0.6);
  }
...
//   const TF1* corrFuncMultSlope = tpcResponse->GetMultSlopeFunction();
//   const TF1* corrFuncMultCurv = tpcResponse->GetMultCurvFunction();
...
//   SetMultiplicityCorrectionFromString(multCorrFactor, &corrFuncMult, &corrFuncMultCurv, &corrFuncSigmaMult);
  ...
  TH2D* hMultCorrFactors = new TH2D("hMultCorrFactors","Multiplicity Correction Factors vs pT", nPbinsForMap, binsPforMap, 400, 0.9,1.5);
...
*/
    ...
    // Same as in checkshapeeta
    //  if (pidType == kTPCandTOFid || pidType == kV0idPlusTOFrejected ) continue; //For testing to get the old result
      ...
      if (pidType != kMCid) {
        if ((pTPC < 0.6 && pidType != kTPCid)               // No V0s/TOF below 0.6 due to bad statistics
          //TODO NOW Maybe don't use this line since the tails get fewer V0's than the main peak, so the tails are overall reduced || (pTPC >= 0.6 && pTPC < pThresholdV0cut && pidType != kTPCandTOFid) || // Do not mix V0's and TOF in this range
            ...
            || (pTPC >= 5 && pidType != kV0idPlusTOFaccepted)) // Additionally require TOF
          continue;
      }

      //if (pidType==5) continue;
      //if (pidType==2) continue;
      //if (pidType==3) continue;

    if (recalculateExpecteddEdx) {
      TSpline3* splPr = 0x0;
      Int_t j=0;
      while (j < nOfMultBins && fMultiplicity > multArrayBins->At(2*j+1)) {
        j++;
      }
      splPr = splinesPr[j];
      if (!splPr) {
        splPr = splinesPr[0];
        std::cout << "Last Mult Bin for splines not filled - this should never happen, check your MultSeparationString!!" << std::endl;
      }
...
//           multCorrectionFactor = tpcResponse->GetMultiplicityCorrectionFast(trackForTheta, dEdxExpected, fMultiplicity);
//           delete trackForTheta;
//           trackForTheta = 0x0;
//         }
//         else {
//           Double_t dEdxExpectedInv = 1. / (dEdxExpected * correctionFactor);
...
//
//           Double_t multCorrectionFactor += relSlope * fMultiplicity;
//
//           New multiplicity method
//           Double_t relSlope = corrFuncMult.Eval(dEdxExpectedInv);
//           Double_t relCurv = corrFuncMultCurv.Eval(dEdxExpectedInv);
//
//           if (fMultiplicity <= -relSlope/(2*relCurv))
//             multCorrectionFactor += relSlope * fMultiplicity + relCurv * fMultiplicity * fMultiplicity;
//           else
//             multCorrectionFactor -= 0.25 *relSlope * relSlope/relCurv;
//         }
//         Double_t relSlope = corrFuncMultSlope->Eval( 1. / (dEdxExpected * correctionFactor));
//         Double_t relCurv = corrFuncMultCurv->Eval( 1. / (dEdxExpected * correctionFactor));
//
//         if (relCurv <= 0.0 || (fMultiplicity <= -relSlope/(2*relCurv)))
//           multCorrectionFactor += relSlope * fMultiplicity + relCurv * fMultiplicity * fMultiplicity;
//         else
//           multCorrectionFactor -= 0.25 *relSlope * relSlope/relCurv;

        AliExternalTrackParam* trackForTheta = 0x0;
//         Double_t covar[15] = {0.0};
//         Double_t param[5] = {0.0};
//         param[3] = TMath::Tan(0.5*TMath::Pi() - TMath::ATan(tanTheta));
//         AliExternalTrackParam* trackForTheta = new AliExternalTrackParam(0.0,0.0,param,covar);
        multCorrectionFactor = tpcResponse->GetMultiplicityCorrectionFast(trackForTheta, dEdxExpected, fMultiplicity);
  //      cout << "MULTIPLICTY CORRECTION IS ON"<< endl;
//         delete trackForTheta;
//         trackForTheta = 0x0;
      }
      hMultCorrFactors->Fill(pTPC, multCorrectionFactor);
...
//       Double_t multSigmaCorrectionFactor = 1. + relSigmaSlope * fMultiplicity;
      Double_t multSigmaCorrectionFactor = tpcResponse->GetMultiplicitySigmaCorrectionFast(dEdxExpected * correctionFactor,fMultiplicity);
//cout << "MULTIPLICTY CORRECTION IS ON2"<< endl;
    ...
    for (Int_t j = 0; j < nMultHistos; j++)    {
      if (fMultiplicity  >= j*multStepSize && fMultiplicity < (j+1)*multStepSize)  {
        hPullAdditionalCorrMult[j]->Fill(pTPC, pull);
      }
    }
  }
  progressbar(100.);
...
*/
  ...
  TCanvas* canvPullSigmaCorrMult = 0x0;
  ...
  if (hMap) {
    ...
    if (nMultHistos > 0) {
      canvPullSigmaCorrMult = new TCanvas("canvPullSigmaCorrMult", "canvPullSigmaCorrMult", 100,10,1380,800);
      canvPullSigmaCorrMult->SetLogx(kTRUE);
      canvPullSigmaCorrMult->SetGridx(kTRUE);
      canvPullSigmaCorrMult->SetGridy(kTRUE);
    }
    ...
    for (Int_t i = 0, j = nThetaHistos; i < nThetaHistos + 1; i++, j--)  {
      ...
      normaliseHisto(hTemp);
      hTemp->FitSlicesY(0,0,-1,0,"QNRG4",0);
      hTemp->GetYaxis()->SetNdivisions(12);
      hTemp->GetXaxis()->SetMoreLogLabels(kTRUE);
      TH1D* hTempMean = (TH1D*)gDirectory->Get(Form("%s_1", hTemp->GetName()));
      hTempMean->SetTitle(Form("mean(pull), %s", thetaString.Data()));
    ...
    for (Int_t i = 0, j= nMultHistos - 1; i < nMultHistos; i++, j--)  {
//       canvPullSigmaCorrMult[i] = new TCanvas(Form("canvPullCorr_%d", i), "canvPullCorr", 100,10,1380,800);
//       canvPullSigmaCorrMult[i]->cd();
//       canvPullSigmaCorrMult[i]->SetLogx(kTRUE);
//       canvPullSigmaCorrMult[i]->SetLogz(kTRUE);
//       canvPullSigmaCorrMult[i]->SetGrid(kTRUE, kTRUE);

      TString multString = Form("%d #leq multiplicity < %d", i*multStepSize, (i+1)*multStepSize);

      TH2D* hTemp = hPullAdditionalCorrMult[i];
      ...
      hTemp->GetXaxis()->SetMoreLogLabels(kTRUE);
      TH1D* hTempMean = (TH1D*)gDirectory->Get(Form("%s_1", hTemp->GetName()));
      hTempMean->SetTitle(Form("mean(pull), %s", multString.Data()));
      hTempMean->GetXaxis()->SetMoreLogLabels(kTRUE);
      hTempMean->SetLineWidth(2);
      hTempMean->SetMarkerStyle(20);
      TH1D* hTempSigma = (TH1D*)gDirectory->Get(Form("%s_2", hTemp->GetName()));
      hTempSigma->SetTitle(Form("#sigma(pull), %s", multString.Data()));
      hTempSigma->GetXaxis()->SetMoreLogLabels(kTRUE);
      hTempSigma->SetLineColor(kMagenta);
      hTempSigma->SetMarkerStyle(20);
      hTempSigma->SetMarkerColor(kMagenta);
      hTempSigma->SetLineWidth(2);
      TH1D* hTempChi2 = (TH1D*)gDirectory->Get(Form("%s_chi2", hTemp->GetName()));
      hTempChi2->SetTitle(Form("#chi^{2} / NDF (pull), %s", multString.Data()));
      hTempChi2->GetXaxis()->SetMoreLogLabels(kTRUE);
      hTempChi2->SetLineColor(kMagenta + 2);
      hTempChi2->SetMarkerStyle(20);
      hTempChi2->SetMarkerColor(kMagenta + 2);
...
//       hTempChi2->DrawCopy((i == 0 ? "" : "same"));
    ...
    if (canvPullSigmaCorrMult)
      canvPullSigmaCorrMult->BuildLegend();
  }

  // added by Jiyoung, just to make pdf file for checking
  //canvPullMeanCorr->SaveAs("checkPullTree_mean.pdf");
  //canvPullSigmaCorr->SaveAs("checkPullTree_sigma.pdf");
  //canvPullChi2Corr->SaveAs("checkPullTree_chi2.pdf");
...
//LHC15o_pass2_lowIR
//   if (isNonPP)
//     printf("Using corr Parameters for LHC15o_pass2_lowIR\n!");
//
//   // width 3, stepsize 800
// //   corrFuncMult.SetParameter(0, -6.232e-07);
// //   corrFuncMult.SetParameter(1, -2.736e-05);
// //   corrFuncMult.SetParameter(2, -6.745e-03);
// //   corrFuncMult.SetParameter(3, 1.901e-02);
// //   corrFuncMult.SetParameter(4, 0);
//   // width 4, stepsize 150 (same as pPb)
//   corrFuncMult.SetParameter(0, -1.077e-06);
//   corrFuncMult.SetParameter(1, -4.999e-05);
//   corrFuncMult.SetParameter(2, -9.812e-03);
//   corrFuncMult.SetParameter(3, 2.492e-02);
//   corrFuncMult.SetParameter(4, 0);
//
//   // width 3, stepsize 800
// //   corrFuncMultTanTheta.SetParameter(0, -4.804e-07);
// //   corrFuncMultTanTheta.SetParameter(1,  1.816e-06);
// //   corrFuncMultTanTheta.SetParameter(2, -0.5);
//   // width 4, stepsize 150 (same as pPb)
//   corrFuncMultTanTheta.SetParameter(0, -5.562e-07);
//   corrFuncMultTanTheta.SetParameter(1,  1.644e-06);
//   corrFuncMultTanTheta.SetParameter(2, -0.5);
//
// //   corrFuncSigmaMult.SetParameter(0, -1.828e-06);
// //   corrFuncSigmaMult.SetParameter(1, 1.617e-03);
// //   corrFuncSigmaMult.SetParameter(2, 5.212e-03);
// //   corrFuncSigmaMult.SetParameter(3, 1.815e-02);
...
//   /*
//   corrFuncMult.SetParameter(0, 0);
//   corrFuncMult.SetParameter(1, 0);
//   corrFuncMult.SetParameter(2, 0);
//   corrFuncMult.SetParameter(3, 0);
//   corrFuncMult.SetParameter(4, 0);
//
//   corrFuncMultTanTheta.SetParameter(0, 0);
//   corrFuncMultTanTheta.SetParameter(1, 0);
//   corrFuncMultTanTheta.SetParameter(2, 0);
//
//   corrFuncSigmaMult.SetParameter(0, 0.);
//   corrFuncSigmaMult.SetParameter(1, 0.);
//   corrFuncSigmaMult.SetParameter(2, 0.);

...
Int_t addMapToFile(TString filePathNameMapToAdd, TString normalisation, TString period, Int_t pass, Bool_t isMC, Bool_t isSigma, 
                   TString pathMapPackage = "etaMaps", TString fileNameMapPackage = "TPCetaMaps.root", Int_t firstRun = 0, Int_t lastRun = 0)
{
  ...
  if (firstRun != 0) {
    //Take run ranges from input
    mapTitle = Form("%s.pass%d",period.Data(), pass);
    ...
    /*else if (period.Contains("LHC13B") == 1 && !isMC) {
      ...
      // In addition, this parametrisation is used as a default
      // from the end of LHC13f until the start of LHC15a
      period = "LHC13B";
    ...
    else if (period.Contains("LHC13C") == 1 && !isMC) {
      //LHC13a-d (beam only since 13b) periods -> 5.023 ATeV (pPb)
      // -> High luminosity periods (e and following) require
      // at the moment separate treatmeant.
      // In addition, this parametrisation is used as a default
      // from the end of LHC13f until the start of LHC15a
      period = "LHC13C";
      mapTitle = Form("LHC13c.pass%d", pass);
    ...
    else if (period.Contains("LHC15F") == 1 && !isMC) {
      //LHC15 pp periods at 13TeV (first beam for LHC15f, but take paras from LHC15a on)
      period = "LHC15F";
      mapTitle = Form("LHC15f.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 208505; 
      runUp[0]  = 227734; //logbook --> w/ beam 226607; w/o beam 227734;
    }
    else if (period.Contains("LHC15G") == 1 && !isMC) {
      //LHC15 pp periods at 13TeV (first beam for LHC15f, but take paras from LHC15a on)
      period = "LHC15G";
      mapTitle = Form("LHC15g.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 227735; // logbook --> w/ beam 228909; w/o beam 227735;
      runUp[0]  = 232411; // logbook --> w/ beam 231326; w/o beam 232411;
    }
    else if (period.Contains("LHC15H") == 1 && !isMC) {
      //LHC15 pp periods at 13TeV
      period = "LHC15H";
      mapTitle = Form("LHC15h.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 232412; // logbook --> w/ beam 232912;  w/o beam 232412;
      runUp[0]  = 235169; // logbook --> w/ beam 234050;  w/o beam 235169;
    }
    else if (period.Contains("LHC15I") == 1 && !isMC) {
      //LHC15 pp periods at 13TeV
      period = "LHC15I";
      mapTitle = Form("LHC15i.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 235170; // logbook --> w/ beam 235192; w/o beam 235170;
      runUp[0]  = 236866; // logbook --> w/ beam 236866; w/o beam 236866;
    }
    else if (period.Contains("LHC15J") == 1 && !isMC) {
      //LHC15 pp periods at 13TeV
      period = "LHC15J";
      mapTitle = Form("LHC15j.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 236867; // logbook --> w/ beam 236892; w/o beam 236867;
      runUp[0]  = 238653; //TODO Needs to be replaced with proper upper bound as soon as a subsequent parametrisation is necessary // logbook --> w/ beam 238627; w/o beam 238653;
    }
    else if (period.Contains("LHC15N") == 1 && !isMC) {
      //LHC15 pp periods at 13TeV
      period = "LHC15N";
      mapTitle = Form("LHC15n.pass%d", pass);
    ...
    else if (period.Contains("LHC15O") == 1 && !isMC) {
      //LHC15 PbPb periods at 5TeV
      period = "LHC15O";
      mapTitle = Form("LHC15o.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 244640; 
      runUp[0]  = 247173; //247173 last run
    }
    else if (period.Contains("LHC16F") == 1 && !isMC) {
      //LHC16 pp periods at 13TeV
      period = "LHC16F";
      mapTitle = Form("LHC16f.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 253610; //
      runUp[0]  = 253987; //253987 last run LHC16k
    }  
    else if (period.Contains("LHC16K") == 1 && !isMC) {
      //LHC16 pp periods at 13TeV
      period = "LHC16K";
      mapTitle = Form("LHC16k.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 256490; //256490
      runUp[0]  = 258860; //258860 last run LHC16k
    }
    else if (period.Contains("LHC16L") == 1 && !isMC) {
      //LHC16 pp periods at 13TeV
      period = "LHC16L";
      mapTitle = Form("LHC16l.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 258861; 
      runUp[0]  = 260199; //260199 last run LHC16l
    }
    else if (period.Contains("LHC16Q") == 1 && !isMC) {
      //LHC16 pPb periods at ?TeV
      period = "LHC16Q";
      mapTitle = Form("LHC16q.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
    //     runLow[0] = 264896; 
    //     runUp[0]  = 265533; //265533 last run LHC16q
      runLow[0] = 267139; 
      runUp[0]  = 999999; //267338 last run LHC16t    
    } 
    else if (period.Contains("LHC16R") == 1 && !isMC) {
      //LHC16 pPb periods at ?TeV
      period = "LHC16R";
      mapTitle = Form("LHC16r.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 265534; 
      runUp[0]  = 266329; //266329 last run LHC16r
    }  
    else if (period.Contains("LHC16S") == 1 && !isMC) {
      //LHC16 pPb periods at ?TeV
      period = "LHC16S";
      mapTitle = Form("LHC16s.pass%d", pass);
      
      runLow = new Int_t[nRunRanges];
      runUp = new Int_t[nRunRanges];
      
      runLow[0] = 266330; 
      runUp[0]  = 267138; //267138 last run LHC16s
    }  
    else if (period.Contains("LHC16T") == 1 && !isMC) {
      //LHC16 pPb periods at ?TeV
      period = "LHC16T";
      mapTitle = Form("LHC16t.pass%d", pass);
